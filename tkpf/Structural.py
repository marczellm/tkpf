...
class Structural(Directive):
    ...
    def add_child(self, parent, classname, attrib, text=None) -> tuple:
        ...
        if text:
            attrib['text'] = text
        directives = {}
        for k in copy(attrib).keys():
            if k in Registry.attributes:
                directives[k] = attrib.pop(k)
    ...
    def inflate(self, parent, classname, widget_name=None, directives={}):
        ...
        if classname in Registry.directives:
            ...
            for k, v in directives.items():
                attr_directive_class = Registry.attributes[k]
                attr_directive = attr_directive_class(v, parent, self)
                kwargs.update(attr_directive.kwargs())
